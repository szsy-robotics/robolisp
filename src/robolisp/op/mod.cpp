...
std::string Mod::get_desc() const
{
    return "mod";
}

std::size_t Mod::get_min_args() const
{
    return 2;
}

std::size_t Mod::get_max_args() const
{
    return ARG_SIZE_INF;
}

void Mod::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
        res_ = list.take();
...
void Mod::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
    {
        res_ = int_num.get();
    ...
    {
        if (int_num.get() != 0)
            res_ = static_cast<float>(std::fmod(std::get<float>(res_), int_num.get()));
        else
            reg_err(val::Err::ARITHMETIC_ERR, "divide by zero");
    }
    else
...
void Mod::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
    {
        res_ = float_num.get();
    ...
    {
        if (float_num.get() != 0)
            res_ = static_cast<float>(std::fmod(std::get<float>(res_), float_num.get()));
        else
            reg_err(val::Err::ARITHMETIC_ERR, "divide by zero");
    }
    else if (std::holds_alternative<int32_t>(res_))
    {
        if (float_num.get() != 0)
            res_ = static_cast<float>(std::fmod(std::get<int32_t>(res_), float_num.get()));
        else
            reg_err(val::Err::ARITHMETIC_ERR, "divide by zero");
    }
    else
...
OpPtr robolisp::impl::op::create_mod(Env */*env*/)
{
    return OpPtr(new Mod());
}
