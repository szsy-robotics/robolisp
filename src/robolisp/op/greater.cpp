...
ValPtr Greater::take_res()
{
    return val::create_sym(res_);
}

std::string Greater::get_desc() const
{
    return "greater";
}

std::size_t Greater::get_min_args() const
{
    return 2;
}

std::size_t Greater::get_max_args() const
{
    return ARG_SIZE_INF;
}

void Greater::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
...
void Greater::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
    {
...
void Greater::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
    {
...
void Greater::process(val::Str &&str)
{
    if (get_arg_cnt() == 1)
    {
...
OpPtr robolisp::impl::op::create_greater(Env */*env*/)
{
    return OpPtr(new Greater());
}
