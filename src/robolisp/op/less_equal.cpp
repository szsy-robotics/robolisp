...
ValPtr LessEqual::take_res()
{
    return val::create_sym(res_);
}

std::string LessEqual::get_desc() const
{
    return "less_equal";
}

std::size_t LessEqual::get_min_args() const
{
    return 2;
}

std::size_t LessEqual::get_max_args() const
{
    return ARG_SIZE_INF;
}

void LessEqual::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
...
void LessEqual::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
    {
...
void LessEqual::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
    {
...
void LessEqual::process(val::Str &&str)
{
    if (get_arg_cnt() == 1)
    {
...
OpPtr robolisp::impl::op::create_less_equal(Env */*env*/)
{
    return OpPtr(new LessEqual());
}
