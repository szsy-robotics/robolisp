...
std::string Pow::get_desc() const
{
    return "pow";
}

std::size_t Pow::get_min_args() const
{
    return 2;
}

std::size_t Pow::get_max_args() const
{
    return ARG_SIZE_INF;
}

void Pow::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
        res_ = list.take();
...
void Pow::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
        res_ = int_num.get();
    else if (std::holds_alternative<float>(res_))
        res_ = std::pow(std::get<float>(res_), int_num.get());
    else
        reg_illegal_arg(int_num);
}

void Pow::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
        res_ = float_num.get();
    else if (std::holds_alternative<float>(res_))
        res_ = std::pow(std::get<float>(res_), float_num.get());
    else
        reg_illegal_arg(float_num);
}

OpPtr robolisp::impl::op::create_pow(Env */*env*/)
{
    return OpPtr(new Pow());
}
