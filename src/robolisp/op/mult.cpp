...
std::string Mult::get_desc() const
{
    return "mult";
}

std::size_t Mult::get_min_args() const
{
    return 2;
}

std::size_t Mult::get_max_args() const
{
    return ARG_SIZE_INF;
}

void Mult::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
        res_ = list.take();
...
void Mult::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
        res_ = int_num.get();
    else if (std::holds_alternative<int32_t>(res_))
...
void Mult::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
        res_ = float_num.get();
    else if (std::holds_alternative<float>(res_))
...
OpPtr robolisp::impl::op::create_mult(Env */*env*/)
{
    return OpPtr(new Mult());
}
