...
ValPtr GreaterEqual::take_res()
{
    return val::create_sym(res_);
}

std::string GreaterEqual::get_desc() const
{
    return "greater_equal";
}

std::size_t GreaterEqual::get_min_args() const
{
    return 2;
}

std::size_t GreaterEqual::get_max_args() const
{
    return ARG_SIZE_INF;
}

void GreaterEqual::process(val::List &&list)
{
    if (get_arg_cnt() == 1)
    {
...
void GreaterEqual::process(val::IntNum &&int_num)
{
    if (get_arg_cnt() == 1)
    {
...
void GreaterEqual::process(val::FloatNum &&float_num)
{
    if (get_arg_cnt() == 1)
    {
...
void GreaterEqual::process(val::Str &&str)
{
    if (get_arg_cnt() == 1)
    {
...
OpPtr robolisp::impl::op::create_greater_equal(Env */*env*/)
{
    return OpPtr(new GreaterEqual());
}
